    ...
    if($team==""){
      ...
      if(isset($_GET["category"])){
        ...
        if($category==""){
          ...
          $stmt->execute([$category]);
          if ($stmt->rowCount() != 0) {
            $stmt = $db->prepare("SELECT articles.id, articles.approvated, articles.limited, articles.title, articles.article, articles.subtitle, articles.date, articles.imgdir, user.nickname, category.categoryName, articles.imgdir, team.teamName FROM articles JOIN user ON user.id=articles.journalist JOIN category ON category.id=articles.category JOIN team ON team.id=articles.team WHERE category.id=? AND articles.approvated=1 ORDER BY articles.date DESC LIMIT 50");
            $stmt->execute([$category]);
            $rowos=array();
            while($row = $stmt->fetch()){
              $rowo=array(
